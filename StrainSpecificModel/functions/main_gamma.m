...
%% Initialize parameters for aggregating simulation                                 
...
parfor simCtr = 1:numberSimulations
    ...
    npr1Worms = initialWorms(:,:,simCtr);                      % initial distribution of worms on lattice
    FUsNpr1 = initialFUs(:,:,simCtr);
...
parfor simCtr = 1:numberSimulations
    ...
    N2worms = initialWorms(:,:,simCtr);  % initial distribution of worms on lattice
    FUsN2 = initialFUs(:,:,simCtr);
...
%     
% v = VideoWriter([savepath 'L' num2str(L) 'N' num2str(N) 'gamma' num2str(gamma) filesuffix],'MPEG-4');
% v.Quality = 100;
% open(v);
% for i = 1:time
